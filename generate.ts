...
const apiSpecs: { [key: string]: string } = {
  kubernetes:
    "https://raw.githubusercontent.com/kubernetes/kubernetes/release-1.17/api/openapi-spec/swagger.json",
};
...
async function run() {
  ...
  for (const apiKey of Object.keys(apiSpecs)) {
    ...
    const apiSpec: OpenAPIV2.Document = (await axios.get(apiSpecs[apiKey]))
      .data;
